...
//		[stationInstance: stationInstance]
	...
	def savePic(Long id){
		   ...
		   def stationPic = request.getFile('stationPic')
		   def station = Station.get(id)
		   if(stationPic && !stationPic.empty){
			   def sp = new StationPicture()
			   sp.station = station
			   StationPicture.findByStationAndEnabled(station,true).each {s->s.enabled = false}
			   sp.enabled = true
			   sp.save(flush:true)
			   def fileName = sp.file
			   stationPic.transferTo(new File("${Holders.config.grails.dynImage.rootPath}${fileName}"))
			   
			   flash.message="图片上传成功。"
			   redirect(action:'show',id:id)
		   }
	}
